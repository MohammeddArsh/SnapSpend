...
export async function render(container, selectedMonth) {
    ...
    container.innerHTML = `
        <div class="max-w-3xl mx-auto flex flex-col" style="height: calc(100vh - 190px); min-height: 420px;">
            <!-- Assistant Header -->
            ...
            <div id="assistant-suggestions" class="flex flex-wrap gap-2 pb-3">
                ${SUGGESTED_QUESTIONS.map(q => `
                    <button class="suggestion-chip text-[11px] font-semibold text-brand-700 dark:text-brand-300 bg-brand-50 dark:bg-brand-950/50 hover:bg-brand-100 dark:hover:bg-brand-900/50 border border-brand-200/70 dark:border-brand-900/60 rounded-full px-3.5 py-1.5 transition-all cursor-pointer">
                        ${escapeHTML(q)}
                    </button>
                `).join('')}
            </div>
            ...
            <div class="glass-surface dark:bg-slate-900/70 border border-slate-200/70 dark:border-slate-700/60 rounded-2xl p-2.5 shadow-lg shadow-slate-900/5 dark:shadow-black/30 flex items-end gap-2">
                ...
                <button id="assistant-send" class="shrink-0 bg-brand-gradient hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl px-4 py-2 text-xs font-bold shadow-lg shadow-indigo-500/25 transition-all flex items-center gap-1.5 cursor-pointer">
            ...
            <div class="max-w-[95%] glass-surface dark:bg-slate-900/70 border border-slate-200/70 dark:border-slate-700/60 rounded-2xl rounded-bl-md px-4 py-3 shadow-sm space-y-2.5">
                ...
                ${isTabular ? `
                    <details class="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 overflow-hidden">
                        <summary class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider px-3 py-2 cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
                            ${rows.length} result row${rows.length === 1 ? '' : 's'}
                        </summary>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-[11px] font-mono">
                                <thead>
                                    <tr class="bg-slate-100 dark:bg-slate-800 text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">
                ...
                ${data.sql ? `
                    <details class="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40 overflow-hidden">
                        <summary class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider px-3 py-2 cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
                            SQL used
                        </summary>
                        <pre class="px-3 pb-2.5 text-[10px] font-mono text-slate-500 dark:text-slate-400 overflow-x-auto whitespace-pre-wrap">${escapeHTML(data.sql)}</pre>
                    </details>
                ` : ''}
            </div>
        </div>
